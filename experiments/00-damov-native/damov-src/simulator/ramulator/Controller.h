...
public:
    ...
    struct TrackCoreRuntimeStats {  // Runtime bookkeeping per tracked core.
        TrackCoreAggregate aggregate;
        bool seenIssue = false;
        uint64_t lastIssueCycle = 0;
    };
    bool trackCoreStatsEnabled = false;
    std::vector<int> trackedCoreIds;
    std::unordered_map<int, size_t> trackedCoreIndex;
    std::vector<TrackCoreRuntimeStats> trackedCoreStats;
    ...
    bool enqueue(Request& req)
    {
        ...
        if (req.type == Request::Type::READ && find_if(writeq.q.begin(), writeq.q.end(),
            ...
            if (trackCoreStatsEnabled) {
                handle_tracked_issue(pending.back(), 0, clk);
            }
            readq.q.pop_back();
        }
    ...
    void tick()
    {
        ...
        if (pending.size()) {
            ...
            if (req.depart <= clk) {
                uint64_t lat = (req.depart >= req.arrive)? req.depart - req.arrive : 0;
                if (lat > 1) { // this request really accessed a row (when a read accesses the same address of a previous write, it directly returns. See how this is handled in enqueue function)
                  (*read_latency_sum) += lat;
                    channel->update_serving_requests(
                      req.addr_vec.data(), -1, clk);
                }
                if (trackCoreStatsEnabled) {
                    handle_tracked_completion(req, lat, false);
                }
        ...
        if (pending_write.size()) {
            ...
            if (req.depart <= clk) {
                uint64_t lat = (req.depart >= req.arrive)? req.depart - req.arrive : 0;
                if (trackCoreStatsEnabled) {
                    handle_tracked_completion(req, lat, true);
                }
                req.callback(req);
        ...
        if (req->is_first_command) {
            uint64_t queueTime = (clk >= req->arrive)? clk - req->arrive : 0;
            if (trackCoreStatsEnabled) {
                handle_tracked_issue(*req, queueTime, clk);
            }
            ...
            if (req->type == Request::Type::READ) {
                (*queueing_latency_sum) += queueTime;
                if (is_row_hit(req)) {
    ...
    void set_tracked_cores(const std::vector<int>& coreIds) {
        trackedCoreIds.assign(coreIds.begin(), coreIds.end());
        trackCoreStatsEnabled = !trackedCoreIds.empty();
        trackedCoreIndex.clear();
        trackedCoreStats.clear();
        if (!trackCoreStatsEnabled) return;
        trackedCoreStats.resize(trackedCoreIds.size());
        for (size_t i = 0; i < trackedCoreIds.size(); ++i) {
            trackedCoreIndex[trackedCoreIds[i]] = i;
            trackedCoreStats[i] = TrackCoreRuntimeStats();
        }
    }

    void accumulate_tracked_core_stats(std::vector<TrackCoreAggregate>& totals) const {
        if (!trackCoreStatsEnabled) return;
        for (size_t i = 0; i < trackedCoreStats.size() && i < totals.size(); ++i) {
            totals[i].readLatencySum += trackedCoreStats[i].aggregate.readLatencySum;
            totals[i].readCount += trackedCoreStats[i].aggregate.readCount;
            totals[i].writeLatencySum += trackedCoreStats[i].aggregate.writeLatencySum;
            totals[i].writeCount += trackedCoreStats[i].aggregate.writeCount;
            totals[i].queueTimeSum += trackedCoreStats[i].aggregate.queueTimeSum;
            totals[i].queueCount += trackedCoreStats[i].aggregate.queueCount;
            totals[i].intervalSum += trackedCoreStats[i].aggregate.intervalSum;
            totals[i].intervalCount += trackedCoreStats[i].aggregate.intervalCount;
        }
    }

private:
    TrackCoreRuntimeStats* get_tracked_stats(int coreid) {
        if (!trackCoreStatsEnabled) return nullptr;
        auto it = trackedCoreIndex.find(coreid);
        if (it == trackedCoreIndex.end()) return nullptr;
        return &trackedCoreStats[it->second];
    }

    void handle_tracked_issue(Request& req, uint64_t queueTime, uint64_t issueCycle) {
        TrackCoreRuntimeStats* stats = get_tracked_stats(req.coreid);
        if (!stats) return;
        stats->aggregate.queueTimeSum += queueTime;
        stats->aggregate.queueCount++;
        if (stats->seenIssue) {
            if (issueCycle >= stats->lastIssueCycle) {
                stats->aggregate.intervalSum += issueCycle - stats->lastIssueCycle;
                stats->aggregate.intervalCount++;
            }
        } else {
    ...
    void handle_tracked_completion(const Request& req, uint64_t latency, bool isWrite) {
        TrackCoreRuntimeStats* stats = get_tracked_stats(req.coreid);
        if (!stats) return;
        if (isWrite) {
            stats->aggregate.writeLatencySum += latency;
            stats->aggregate.writeCount++;
        } else {
            stats->aggregate.readLatencySum += latency;
            stats->aggregate.readCount++;
        }
    }
