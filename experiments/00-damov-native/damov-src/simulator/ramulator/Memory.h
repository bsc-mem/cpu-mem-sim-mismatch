    ...
    void finish() {
      ...
      if (trackCoreStatsEnabled) {
        for (size_t i = 0; i < trackedCoreStatHandles.size(); ++i) {
          ...
          double avgRead = totals.readCount ? static_cast<double>(totals.readLatencySum) / totals.readCount : 0.0;
          double avgWrite = totals.writeCount ? static_cast<double>(totals.writeLatencySum) / totals.writeCount : 0.0;
          double avgQueue = totals.queueCount ? static_cast<double>(totals.queueTimeSum) / totals.queueCount : 0.0;
          double avgInterval = totals.intervalCount ? static_cast<double>(totals.intervalSum) / totals.intervalCount : 0.0;
