...
class StatList {
 ...
 public:
  ...
  void output(std::string filename) {
    ...
    stat_output.exceptions(std::ofstream::failbit | std::ofstream::badbit);
    try {
        stat_output.open(filename, std::ios_base::out);
...
extern StatList* active_statlist;

class StatListScope {
 private:
  StatList* prev_;
 public:
  explicit StatListScope(StatList* new_list) : prev_(active_statlist) {
    active_statlist = new_list ? new_list : active_statlist;
  }
  ~StatListScope() { active_statlist = prev_; }
};
