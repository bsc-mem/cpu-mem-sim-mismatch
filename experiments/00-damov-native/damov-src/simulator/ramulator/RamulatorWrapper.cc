...
RamulatorWrapper::RamulatorWrapper(const char* config_path, unsigned num_cpus, int cacheline, bool pim_mode, bool record_memory_trace, const char* application_name, bool networkOverhead, const std::vector<uint32_t>& trackedCores)
{
    ...
    // embed tracked core config.
    if (!trackedCores.empty()) {
        std::vector<int> cfgCores;
        cfgCores.reserve(trackedCores.size());
        for (uint32_t coreId : trackedCores) {
            cfgCores.push_back(static_cast<int>(coreId));
        }
        configs.set_track_cores(cfgCores);
    }
